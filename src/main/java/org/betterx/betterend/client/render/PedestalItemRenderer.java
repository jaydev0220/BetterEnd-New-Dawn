...
public class PedestalItemRenderer<T extends PedestalBlockEntity> implements BlockEntityRenderer<T, PedestalItemRenderer.PedestalRenderState> {
    ...
    private static void extractHints(PedestalBlockEntity blockEntity, PedestalRenderState state, float partialTick) {
        ...
        if (InfusionHint.isActiveAt(here)) {
            ...
            if (outline != null && outline[0] > 0.0F) {
                ...
                for (int i = 0; i < present.length; i++) {
                    ...
                    state.outlines.add(new HintOutline(
                            shape,
                            InfusionRitual.socketPos(BlockPos.ZERO, i),
                            rgb,
                            alpha
                    ));
                }
        ...
        for (BlockPos crystalPos : EternalHint.pedestalPositions()) {
            state.crystalOffsets.add(crystalPos.subtract(here));
        }
        state.crystalGhostAlpha = intensity * CRYSTAL_GHOST_ALPHA;
    }

    private static void submitHints(
            PedestalRenderState state,
            PoseStack matrices,
            SubmitNodeCollector submitNodeCollector
    ) {
        for (HintOutline outline : state.outlines) {
            submitNodeCollector.submitCustomGeometry(
                    ...
                    (pose, vertexConsumer) -> renderShapeOutline(pose, vertexConsumer, outline)
            );
        }
    ...
    private static void renderShapeOutline(
        ...
        outline.shape().forAllEdges((x1, y1, z1, x2, y2, z2) -> {
            ...
            vertexConsumer.addVertex(pose, (float) x1 + offset.getX(), (float) y1 + offset.getY(), (float) z1 + offset.getZ())
                          .setColor(color)
                          .setNormal(pose, nx, ny, nz)
                          .setLineWidth(2.0F);
            vertexConsumer.addVertex(pose, (float) x2 + offset.getX(), (float) y2 + offset.getY(), (float) z2 + offset.getZ())
                          .setColor(color)
                          .setNormal(pose, nx, ny, nz)
                          .setLineWidth(2.0F);
        });
